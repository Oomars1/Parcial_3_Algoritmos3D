...
void drawCylinder(float height, float radius, int sides)
{
    ...
    for (i = 0; i <= sides; i++)
    {
        float x = radius * cos(i * angle) - 1.35f;
...
void keyboardFunc(unsigned char key, int x, int y) {
    switch (key) {
        case 'j':
            angleY -= 5.0f; // Decrementa el ángulo de rotación en el eje X
            break;
        case 'k':
            angleY += 5.0f; // Incrementa el ángulo de rotación en el eje X
            break;
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Rectángulo en 3D en OpenGL");
    ...
    glutKeyboardFunc(keyboardFunc); // Registra la función de control de teclado
